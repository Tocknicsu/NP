...
#define F_PROXY_CONNECTING 1
#define F_PROXY_WRITING 2
#define F_READING 10
#define F_WRITING 11
#define F_DONE 12
...
void client(){
    ...
    for(int i = 1 ; i < 6 ; i++){
		...
		if(env[host].size()){
			...
			string sock_server = "sh" + to_string(i);
			string sock_port = "sp" + to_string(i);
			...
			if(env[sock_server].size()){
				client_sin[i].sin_family = AF_INET;
				client_sin[i].sin_addr = *((in_addr*)gethostbyname(env[sock_server].c_str())->h_addr);
				client_sin[i].sin_port = htons(atoi(env[sock_port].c_str()));
				connect(client_fd[i], (sockaddr*)&client_sin[i], sizeof(client_sin[i]));
				client_status[i] = F_PROXY_CONNECTING;
    ...
    while (conn > 0) {
        ...
        for(int i = 1 ; i < 6 ; i++){
            ...
            } else if (client_status[i] == F_PROXY_CONNECTING && FD_ISSET(client_fd[i], &wfds)) {
                int error;
                socklen_t len = sizeof(int);
                if(getsockopt(client_fd[i], SOL_SOCKET, SO_ERROR, (char*)&error, &len) < 0 || error != 0){
                    client_status[i] = F_DONE;
                    ...
                    cerr << "connect server." << endl;
                    client_status[i] = F_PROXY_WRITING;
					buffer_sented[i] = 0;
					string host = string("h")+to_string(i);
					string port = string("p")+to_string(i);
					sent_buffer[i][0] = 0x04;
					sent_buffer[i][1] = 0x01;
					sent_buffer[i][2] = stoi(env[port]) / 256;
					sent_buffer[i][3] = stoi(env[port]) % 256;
					sent_buffer[i][8] = 0;
					int ip[4];
					sscanf(inet_ntoa(*((in_addr*)gethostbyname(env[host].c_str())->h_addr)), "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
					sent_buffer[i][4] = ip[0];
					sent_buffer[i][5] = ip[1];
					sent_buffer[i][6] = ip[2];
					sent_buffer[i][7] = ip[3];
                }
            } else if (client_status[i] == F_PROXY_WRITING && FD_ISSET(client_fd[i], &wfds)) {
				cerr << "socket " << strlen(sent_buffer[i]) << endl;
